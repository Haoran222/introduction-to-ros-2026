...
constexpr uint32_t kPacketMagic = 0xDEADC0DE;
...
public:
  UnityTCPStreamReceiver() : Node("Unity_ROS_message_Rx") {
    // Unity connects to this ROS-side TCP server.
  ...
  void Run() {
    ...
    RCLCPP_INFO(
        get_logger(),
        "Waiting for Unity TCP connection on %s:%d",
    ...
    // One parser is registered for each message type supported by the bridge.
    std::vector<std::shared_ptr<UnityStreamParser>> stream_parsers(
        UnityMessageType::MESSAGE_TYPE_COUNT);
    ...
    while (rclcpp::ok() && stream_reader->Good()) {
      try {
        ...
        if (header.type < UnityMessageType::MESSAGE_TYPE_COUNT && stream_parsers[header.type]) {
          ...
          RCLCPP_WARN(
              get_logger(),
              "Message type %u is known but not implemented in this bridge",
              static_cast<unsigned>(header.type));
        } else {
          RCLCPP_ERROR(
              get_logger(),
              "Received unknown Unity message type %u",
              static_cast<unsigned>(header.type));
        }
      } catch (const TCPStreamReaderException &e) {
...
private:
  void LoadConfigParameters() {
    ...
    if (!file.is_open()) {
      RCLCPP_WARN(
          get_logger(),
          "Config file not found at %s; using node parameters/defaults",
          config_path.c_str());
      return;
    }
      ...
      if (config.contains("sensorConfig")) {
        if (config["sensorConfig"].contains("port")) {
          set_parameter(rclcpp::Parameter(
              "port",
              static_cast<int>(config["sensorConfig"]["port"])));
        }

        // Accept Unity connections on all interfaces.
      }
    } catch (const std::exception &e) {
      RCLCPP_WARN(
          get_logger(),
