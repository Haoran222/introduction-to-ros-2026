...
int main(int argc, char **argv) {
  ...
  try {
    ...
    RCLCPP_ERROR(
        node->get_logger(),
        "JSON reader failed. Is the configuration file correctly structured?");
    rclcpp::shutdown();
    return 1;
  }

  // Expose the two socket endpoints as ordinary ROS parameters for inspection.
  try {
    ...
    RCLCPP_INFO(
        node->get_logger(),
        "Read control message Tx address: %s, port: %d",
        ctrl_addr.c_str(), ctrl_port);
  } catch (const std::exception &e) {
    RCLCPP_ERROR(
        node->get_logger(),
        "controlConfig is incomplete or invalid in %s: %s",
  ...
  try {
    ...
    RCLCPP_INFO(
        node->get_logger(),
        "Read sensor message Rx address: %s, port: %d",
        sensor_addr.c_str(), sensor_port);
  } catch (const std::exception &e) {
    RCLCPP_ERROR(
        node->get_logger(),
        "sensorConfig is incomplete or invalid in %s: %s",
        config_path.c_str(), e.what());
  }
