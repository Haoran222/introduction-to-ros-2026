...
class UDPClient {
public:
  // Resolve once and keep one UDP socket for the whole process lifetime.
...
public:
  ROSCommandTransmitter() : Node("ROS_Unity_command_Tx") {
    // Defaults are overridden by controlConfig when the JSON is available.
  ...
  void Run() {
    ...
    RCLCPP_INFO(
        get_logger(),
        "Starting command transmit to Unity via UDP %s:%d",
        address.c_str(), port);
...
private:
      ...
      if (config.contains("controlConfig")) {
        if (config["controlConfig"].contains("address")) {
          set_parameter(rclcpp::Parameter(
              "address",
              static_cast<std::string>(config["controlConfig"]["address"])));
        }

        if (config["controlConfig"].contains("port")) {
          set_parameter(rclcpp::Parameter(
              "port",
              static_cast<int>(config["controlConfig"]["port"])));
        }
      }
  ...
  // Latest command is resent at 100 Hz, matching Unity's UDP interface.
